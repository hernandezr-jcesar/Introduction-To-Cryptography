...
def export_public_key(public_key1,filename):
    with open(filename,"wb") as file:
        ...
        file.close
def Firmar():
    filename = filedialog.askopenfilename(initialdir = "C:/Users/HomeDocuments/6.Sexto/Crypto/CryP2",title = "Select a File",filetypes = (("Text files","*.txt*"),("all files","*.*")))
    ...
    privatekey = RSA.import_key(open(archivo[0]+"/LlavePrivada/private_key.pem", "rb").read())
...
def Llave():
    ...
    export_private_key(keypair,"LlavePrivada/private_key.pem")
    export_public_key(public_key,"LlavesPublicas/public_key.pem")
    messagebox.showinfo(title="Llaves",message="Par de llaves generado")
window=Tk()
window.geometry("300x150+100+100")
window.title("Práctica 2")
btnftxt=Button(window,text="Firmar",font=("Courier New",12),width=30,height=2,command=Firmar).grid(row=0,column=0)
btnctxt=Button(window,text="Corroborar",font=("Courier New",12),width=30,height=2,command=Corroborar).grid(row=1,column=0)
btnkey=Button(window,text="Generar Llave",font=("Courier New",12),width=30,height=2,command=Llave).grid(row=2,column=0)
window.mainloop()